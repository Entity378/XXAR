...
class BaseBrowserHandler:
            ...
            for track_id, offsets in offsets_by_track.items():
                ...
                for offset in offsets:
                    ...
                    if pos_in_remaining != -1:
                        ...
                        pattern_matches = (
                            pattern_matches and existing_after_pattern == duration_bytes
                        )
                        if pos >= 28:
                            f.seek(pos - 28)
                            existing_before_pattern = f.read(8)
                            pattern_matches = (
                                pattern_matches and existing_before_pattern == duration_bytes
                            )
    ...
    def _iter_loop_bank_files(self, streaming_root):
        all_pcks = sorted(Path(streaming_root).rglob("*.pck"), key=lambda p: p.as_posix())

        regex = getattr(self.game, "bank_pck_regex", None)
        if regex:
            compiled = re.compile(regex, re.IGNORECASE)
            return [p for p in all_pcks if compiled.match(p.name)]

        glob_pattern = str(getattr(self.game, "soundbank_pck_glob", "") or "").strip()
        if not glob_pattern or glob_pattern == "*.pck":
            prefix = str(getattr(self.game, "soundbank_pck_prefix", "") or "").strip()
            if prefix:
                return [p for p in all_pcks if p.name.lower().startswith(prefix.lower())]
            return list(all_pcks)

        return [p for p in all_pcks if fnmatch(p.name, glob_pattern)]

    @staticmethod
    def _streaming_to_persistent_dir(path):
        return Path(str(path).replace("StreamingAssets", "Persistent"))
    ...
    def apply_post_pack_steps(self, replacements):
        ...
        streaming_root = (
            ...
            if self.bridge and getattr(self.bridge, "_audio_root", None)
            else None
        )
        if not streaming_root or not streaming_root.exists():
            raise FileNotFoundError(
                QCoreApplication.translate(
                    "Application",
                    "Could not locate Streaming audio folder for loop point patching.",
                )
            )

        bank_files = self._iter_loop_bank_files(streaming_root)
        if not bank_files:
            self._emit_status(
                QCoreApplication.translate(
                    "Application",
                    "No bank PCK files found for loop point patching.",
                )
            )
            return {"patched_files": 0, "patched_ids": 0}
        ...
        for bank_source in bank_files:
            bank_target_dir = self._streaming_to_persistent_dir(bank_source.parent)
            ...
            if not bank_target.exists():
                bank_target.write_bytes(bank_source.read_bytes())
            elif scan_source != bank_target:
                bank_target.write_bytes(scan_source.read_bytes())
        ...
        if result["patched_files"] > 0:
            self._emit_status(
                QCoreApplication.translate(
                    "Application",
                    "Loop points patched in %1 bank file(s) for %2 track ID(s).",
                )
    ...
    def apply_post_mod_manager_steps(
        ...
        try:
            ...
            handler = cls(bridge=None)
            handler._status_callback = status_callback

        if not handler._loop_point_supported():
            return {"patched_files": 0, "patched_ids": 0}
        ...
        if not persistent_root:
            raise FileNotFoundError(
                QCoreApplication.translate(
                    "Application",
                    "Could not locate Persistent audio folder for loop point patching.",
                )
            )
        ...
        bank_files = handler._iter_loop_bank_files(streaming_root)
        if not bank_files:
        ...
        for bank_source in bank_files:
            ...
            if base_file != bank_target:
                bank_target.write_bytes(base_file.read_bytes())
